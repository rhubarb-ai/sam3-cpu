...
class Sam3VideoPredictor:
    ...
    def _get_session_stats(self):
        ...
        if torch.cuda.is_available():
            ...
            memory_str = f"CPU memory: {psutil.Process().memory_info().rss // 1024**2} MiB used"
        session_stats_str = f"live sessions: [{', '.join(live_session_strs)}], {memory_str}"
    ...
    def _get_torch_and_gpu_properties(self):
        ...
        if torch.cuda.is_available():
            ...
            torch_and_gpu_str = f"torch: {torch.__version__} with CPU device"
...
class Sam3VideoPredictorMultiGPU(Sam3VideoPredictor):
    def __init__(self, *model_args, gpus_to_use=None, **model_kwargs):
        if not torch.cuda.is_available():
            raise RuntimeError(
                "Sam3VideoPredictorMultiGPU requires CUDA. "
                "Use Sam3VideoPredictor for CPU inference."
            )
...
class Sam3VideoPredictorMultiCPU(Sam3VideoPredictor):
    """Multi-process CPU predictor for parallel inference across multiple CPU cores."""
    
    def __init__(self, *model_args, num_workers=None, **model_kwargs):
        if torch.cuda.is_available():
            logger.warning(
        ...
        IS_MAIN_PROCESS = os.getenv("IS_MAIN_PROCESS", "1") == "1"
        if IS_MAIN_PROCESS:
            logger.info(f"using {num_workers} CPU workers for parallel inference")
            assert num_workers > 0 and isinstance(num_workers, int)
            os.environ["MASTER_ADDR"] = "localhost"
            os.environ["MASTER_PORT"] = f"{self._find_free_port()}"
            os.environ["RANK"] = "0"
            os.environ["WORLD_SIZE"] = f"{num_workers}"
        ...
        logger.info(f"loading model on {self.rank_str} -- this could take a while ...")
        # Force device to CPU for all workers
        model_kwargs['device'] = 'cpu'
        super().__init__(*model_args, **model_kwargs)
        logger.info(f"loading model on {self.rank_str} -- DONE locally")
        
        if self.world_size > 1 and self.rank == 0:
            # Start worker processes after the model is loaded in the main process
            self._start_worker_processes(*model_args, **model_kwargs)
            for rank in range(1, self.world_size):
                self.command_queues[rank].put(("start_gloo_process_group", None))
            self._start_gloo_process_group()
    ...
    def handle_request(self, request):
        ...
        # When starting a session, create a session id before dispatching
        if request["type"] == "start_session" and request.get("session_id") is None:
            request["session_id"] = str(uuid.uuid4())
    ...
    def _start_worker_processes(self, *model_args, **model_kwargs):
        """Start worker processes for handling model inference on CPU."""
        world_size = self.world_size
        logger.info(f"spawning {world_size - 1} worker processes")
        # Use "spawn" for clean process isolation
        mp_ctx = mp.get_context("spawn")
        self.command_queues = {rank: mp_ctx.Queue() for rank in range(1, world_size)}
        self.result_queues = {rank: mp_ctx.Queue() for rank in range(1, world_size)}
        parent_pid = os.getpid()
        ...
        for rank in range(1, world_size):
            ...
            os.environ["IS_MAIN_PROCESS"] = "0"  # mark as worker process
            os.environ["RANK"] = f"{rank}"
            worker_process = mp_ctx.Process(
                target=Sam3VideoPredictorMultiCPU._worker_process_command_loop,
                args=(
                    rank,
                    world_size,
                    self.command_queues[rank],
        ...
        # Revert environment variables for the main process
        os.environ["IS_MAIN_PROCESS"] = "1"
        os.environ["RANK"] = "0"
        
        # Wait for all worker processes to load the model and collect their PIDs
        self.worker_pids = {}
        for rank in range(1, self.world_size):
            # Large timeout to cover potentially long model loading time
            _, worker_pid = self.result_queues[rank].get(timeout=7200)
            self.worker_pids[rank] = worker_pid
        logger.info(f"spawned {world_size - 1} worker processes")
    
    def _start_gloo_process_group(self):
        """Start Gloo process group for CPU-based distributed processing."""
        rank = int(os.environ["RANK"])
        world_size = int(os.environ["WORLD_SIZE"])
        if world_size == 1:
            return
        
        logger.debug(f"starting Gloo process group on {rank=} with {world_size=}")
        assert not torch.distributed.is_initialized()
        # Use the "env://" init method with environment variables
        timeout_sec = int(os.getenv("SAM3_COLLECTIVE_OP_TIMEOUT_SEC", "180"))
        timeout = datetime.timedelta(seconds=timeout_sec)
        torch.distributed.init_process_group(
        ...
        # Warm-up the Gloo process group by running a dummy all-reduce
        tensor = torch.ones(1024, 1024, device=self.device)
        torch.distributed.all_reduce(tensor)
        logger.debug(f"started Gloo process group on {rank=} with {world_size=}")
    ...
    def _worker_process_command_loop(
        ...
        """Command loop for each worker process on CPU."""
        logger.info(f"starting CPU worker process {rank=} with {world_size=}")
        ...
        # Load the model in this worker process
        predictor = Sam3VideoPredictorMultiCPU(
            *model_args, num_workers=num_workers, **model_kwargs
        )
        logger.info(f"started CPU worker {rank=} with {world_size=}")
        
        # Return worker process ID to main process
        worker_pid = os.getpid()
        result_queue.put(("load_model", worker_pid))
        
        # Wait for command to start Gloo process group
        request_type, _ = command_queue.get(timeout=7200)
        assert request_type == "start_gloo_process_group"
        predictor._start_gloo_process_group()
        ...
        while True:
            try:
                request, is_stream_request = command_queue.get(timeout=5.0)
                if request == "shutdown":
                    logger.info(f"CPU worker {rank=} shutting down")
                    torch.distributed.destroy_process_group()
                    result_queue.put(("shutdown", True))
                    sys.exit(0)
                
                logger.debug(f"CPU worker {rank=} received request {request['type']=}")
                if is_stream_request:
                    for _ in predictor.handle_stream_request(request):
                        pass  # Handle stream requests in generator fashion
                else:
                    predictor.handle_request(request)
            except queue.Empty:
                ...
                if not psutil.pid_exists(parent_pid):
                    logger.info(
                        f"stopping CPU worker {rank=} as parent process has exited"
                    )
                    sys.exit(1)
            except Exception as e:
                logger.error(f"CPU worker {rank=} exception: {e}", exc_info=True)
    
    def shutdown(self):
        """Shutdown all worker processes."""
        if self.rank == 0 and self.world_size > 1:
            logger.info(f"shutting down {self.world_size - 1} CPU worker processes")
            for rank in range(1, self.world_size):
                self.command_queues[rank].put(("shutdown", False))
            torch.distributed.destroy_process_group()
            for rank in range(1, self.world_size):
                self.result_queues[rank].get()  # Wait for worker to acknowledge
            logger.info(f"shut down {self.world_size - 1} CPU worker processes")
        self.has_shutdown = True
